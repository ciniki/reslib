...
// -------
//
function ciniki_reslib_categoryDelete(&$ciniki) {
    //
    ...
    $rc = ciniki_reslib_checkAccess($ciniki, $args['tnid'], 'ciniki.reslib.categoryDelete');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( !isset($rc['category']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.reslib.23', 'msg'=>'Category does not exist.'));
    }
    ...
    if( isset($rc['num']['items']) && $rc['num']['items'] > 0 ) {
        $count = $rc['num']['items'];
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.reslib.48', 'msg'=>'There ' . ($count==1?'is':'are') . ' still ' . $count . ' subcategor' . ($count==1?'y':'ies') . ' in the category.'));
    }
    ...
    $rc = ciniki_core_objectCheckUsed($ciniki, $args['tnid'], 'ciniki.reslib.category', $args['category_id']);
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.reslib.24', 'msg'=>'Unable to check if the category is still being used.', 'err'=>$rc['err']));
    }
    if( $rc['used'] != 'no' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.reslib.25', 'msg'=>'The category is still in use. ' . $rc['msg']));
    }
    ...
    // Remove the category
    //
    $rc = ciniki_core_objectDelete($ciniki, $args['tnid'], 'ciniki.reslib.category',
