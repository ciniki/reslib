...
// -------
//
function ciniki_reslib_sectionDelete(&$ciniki) {
    //
    ...
    $rc = ciniki_reslib_checkAccess($ciniki, $args['tnid'], 'ciniki.reslib.sectionDelete');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( !isset($rc['section']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.reslib.16', 'msg'=>'Section does not exist.'));
    }
    $section = $rc['section'];
    ...
    if( isset($rc['num']['items']) && $rc['num']['items'] > 0 ) {
        $count = $rc['num']['items'];
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.reslib.44', 'msg'=>'There ' . ($count==1?'is':'are') . ' still ' . $count . ' categor' . ($count==1?'y':'ies') . ' in the section.'));
    }
    ...
    $rc = ciniki_core_objectCheckUsed($ciniki, $args['tnid'], 'ciniki.reslib.section', $args['section_id']);
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.reslib.17', 'msg'=>'Unable to check if the section is still being used.', 'err'=>$rc['err']));
    }
    if( $rc['used'] != 'no' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.reslib.18', 'msg'=>'The section is still in use. ' . $rc['msg']));
    }
    ...
    // Remove the section
    //
    $rc = ciniki_core_objectDelete($ciniki, $args['tnid'], 'ciniki.reslib.section',
